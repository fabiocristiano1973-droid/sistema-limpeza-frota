...
import { listarValores } from "@/lib/db";
...
export default function ListaValoresPage() {
  ...
  return (
    <div className="px-4 pb-10 pt-4">
      <Cabecalho titulo="Meus valores" subtitulo="Ainda em validação até virarem prática consistente" voltarPara="/mais" />

      <div className="mt-4">
        <LinkBotao href="/valores/novo">+ Novo valor</LinkBotao>
      </div>

      <div className="mt-5 space-y-3">
        {valores.length === 0 && <EstadoVazio titulo="Nenhum valor cadastrado" />}
        {valores.map((v) => (
          <Link key={v.id} href={`/valores/${v.id}`}>
            <Cartao>
